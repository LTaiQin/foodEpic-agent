...
- 2026-06-07 当前结果：`110 passed, 304 deselected`
- 相比本轮进入专项时的起点 `107 passed, 300 deselected`，当前阶段性增量为 `+3 passed`
...
- `graph_agent` 语义规则层：约 `75%`
- `planner` why 补证据路由：约 `55%`
- `verifier / finalize` 收口：约 `55%`
- `toolbox` why sufficiency 对齐：约 `60%`
- 真实小样本 replay 与分 bucket 统计：约 `15%`

综合判断：

- 当前 why 逻辑专项约完成 `60%-65%`
...
- [x] 重新跑 why 专项回归
- [x] 记录当前通过数：`110 passed, 304 deselected`
- [x] 将基线结果写入本清单
- [ ] 建立后续 replay 统一结果模板

进展补充：

- [x] 本轮代码推进后，专项回归已从 `107 passed, 300 deselected` 提升到 `110 passed, 304 deselected`
...
2. `Phase 2 / P2.1`：why 冲突 -> 补证据路由表
3. `Phase 2 / P2.3`：cleaning-precondition 优先补前置帧
4. `Phase 2 / P2.2`：future-use 优先补结果帧
5. `Phase 2 / P2.4`：hidden-access / pairwise 优先补动作后后果
6. `Phase 3 / P3.1-P3.3`：finish 阻断与 sufficiency 收紧
7. `Phase 4 / P4.2-P4.3`：toolbox sufficiency 与 precondition 约束对齐
8. `Phase 5 / P5.1-P5.3`：真实 replay 与 bucket 统计

原因：

- `graph_agent` 纯语义层已经有明显基础；
- 当前收益最大的不是继续堆局部 helper，而是让 planner 与 verifier 真正闭环；
- 只有先把“补什么证据”和“什么时候不允许 finish”做对，后面 replay 才有研究意义。
...
- [ ] `Phase 2 / P2.1`：建立 why 冲突 -> 补证据路由表

本轮默认目标：

- 把当前已经存在的 `precontext / followup / ext2 / pending-resolution` 逻辑，从“局部成立”推进到“按语义 bucket 稳定选路”。

本轮硬要求：

- [ ] 至少新增 `2-4` 条 planner 测试
- [ ] 至少覆盖 `precontext` 和 `followup` 两种不同路由
- [ ] 至少覆盖一个 pending-resolution 回到专用 tool 的场景
- [ ] 跑定向 planner 测试

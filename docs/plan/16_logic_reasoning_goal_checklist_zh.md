...
- 2026-06-07 当前结果：`120 passed, 330 deselected`
...
- [x] 重新跑 why 专项回归
- [x] 记录当前通过数：`120 passed, 330 deselected`
- [x] 将基线结果写入本清单
- [ ] 建立后续 replay 统一结果模板

进展补充：

- [x] 本轮代码推进后，专项回归已从 `107 passed, 300 deselected` 提升到 `120 passed, 330 deselected`
...
- [x] 新增并通过 `6` 条 `future_use` hierarchy 定向测试：
...
- [x] 定向回归：`pytest -q tests/test_graph_agent.py -k 'future_use_causal_hierarchy or future_use_sufficiency'`
  - 结果：`26 passed, 424 deselected`
- [x] 专项总回归：`pytest -q tests/test_graph_agent.py -k 'action_intent'`
  - 结果：`120 passed, 330 deselected`
...
- [ ] `tap state switch / phase-switch` vs `generic full container`
...
2. `Phase 4 / P4.2`：future-use causal hierarchy 继续扩到 `measurement / weigh / open-close / same-object role-use`
3. `Phase 2 / P2.1`：why 冲突 -> 补证据路由表统一入口继续整理成单入口
4. `Phase 3 / P3.4-P3.5`：finalize/conflict filtering 统一收口
5. `Phase 2 / P2.4`：hidden-access / pairwise 的后果帧门控做真实 replay 验证
6. `Phase 5 / P5.1-P5.3`：真实 replay 与 bucket 统计

原因：

- `graph_agent` 纯语义层已经有明显基础；
- `pairwise`、cleaning-specific `future_use`、set-aside / inspection / reuse-vs-placement 这几组高频残差已经推进过一轮；
- 当前更值得继续压的是 `measurement / weigh / open-close / same-object role-use` 这组还主要停留在 sufficiency 层、尚未完全进入 hierarchy 对齐的 future-use 残差；
...
- [ ] `Phase 4 / P4.2`：继续扩展 future-use causal hierarchy 到 `measurement / weigh / open-close / same-object role-use`

本轮默认目标：

- 把当前已经有的 `future_use` hierarchy，继续扩到还没真正对齐到 hierarchy 层的高频语义簇：
  - 明确称量/称重准备时，优先判到 `measurement-base placement / immediate measurement use`
  - 明确同一对象马上要被打开/拧开/揭盖时，优先判到 `same-object open/uncap`
  - 避免 broad `serve / empty / generic move / free-hand` 抢走本该属于精确后续用途的答案

本轮硬要求：

- [ ] 至少新增 `2-4` 条 toolbox / graph 语义对齐测试
- [ ] 至少覆盖 `measurement-base placement`、`same-object open/uncap` 两类不同冲突
- [ ] 至少覆盖一个 `broad serve/empty/move` 被精确 future-use 翻正的场景

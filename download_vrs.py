#!/usr/bin/env python3
"""Download VRS files from the HD-EPIC dataset."""
...
def load_sizes(entries: list[tuple[str, str, str]], cache_path: Path, proxy: str, workers: int) -> dict[str, int]:
    cache = {}
    ...
    if missing:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(workers, 16)) as executor:
            future_to_rel = {executor.submit(get_remote_size, url, proxy): rel for rel, url in missing}
...
def progress_reporter(stop_event, output_root: Path, entries, sizes, interval: int):
    ...
    while not stop_event.wait(interval):
        ...
        complete_files = sum(
            1 for _, rel, _ in entries
            if (output_root / "HD-EPIC" / rel).exists()
            and (output_root / "HD-EPIC" / rel).stat().st_size == sizes[rel]
        )
        part_files = sum(
            1 for _, rel, _ in entries
            if (output_root / "HD-EPIC" / rel).with_name(Path(rel).name + ".part").exists()
        )
        print(
...
def main():
    ...
    parser.add_argument("--size-cache", default="/22liushoulong/agent/hd-epic/data/vrs_sizes.json")
    ...
    sizes = load_sizes(entries, Path(args.size_cache), args.proxy, args.workers)
    total_size = sum(sizes.values())
    print(f"Total VRS size: {sizeof_fmt(total_size)}")
    print(f"Output: {output_root / 'HD-EPIC' / 'VRS'}")
